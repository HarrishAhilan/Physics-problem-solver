...
- Wrap inline math in single dollar signs: $v = 10~\mathrm{m/s}$
- Wrap display equations in double dollar signs: $$F = ma$$
- Use proper LaTeX subscripts: $v_0$, $a_x$, $F_{\mathrm{net}}$
- Use proper LaTeX superscripts: $x^2$, $v^2$
- For units, use \mathrm{} and ~ for spacing: $10~\mathrm{m/s}^2$
- Use \vec{} for vectors: $\vec{F}$, $\vec{v}$
- For words in subscripts use \mathrm{}: $F_{\mathrm{friction}}$

CORRECT Examples:
✓ $v = 5~\mathrm{m/s}$
✓ $a = 9.8~\mathrm{m/s}^2$
✓ $F_{\mathrm{net}} = 50~\mathrm{N}$
✓ $$v_f^2 = v_0^2 + 2a\Delta x$$

INCORRECT Examples (DO NOT USE):
✗ $v = 5 \text{ m/s}$ (don't use \text{})
✗ v = 5 m/s (missing dollar signs)
✗ $v = 5 m/s$ (missing spacing and \mathrm{})
...
3. **List Given Information**: 
   ...
   - Use LaTeX for all variables and values: $m = 5~\mathrm{kg}$, $\theta = 30°$

4. **Determine Relevant Concepts**: 
   - Identify the physics principles and equations needed
   - Write equations in LaTeX: $$F_{\mathrm{net}} = ma$$
...
6. **Final Answer**: 
   ...
   - Example: $$v_f = 15.3~\mathrm{m/s}$$

Keep explanations clear and concise. Use LaTeX for ALL math. Always use \mathrm{} for units with ~ spacing. Provide diagram descriptions when helpful.
